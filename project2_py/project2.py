...
def optimize(f, g, c, x0, n, count, prob):
    ...
    if prob != "secret2":
        ...
        m       = c(x0).size
        lam     = np.zeros(m)       # λᵢ ≥0
        mu      = p['mu0']          # penalty weight
        eps_fd  = 1e-6               # FD for ∇c

        x_best  = x.copy()
        f_best  = np.inf

        # Outer loops update (λ, μ)
        for _ in range(p['outer']):
            # Inner PGD on L(x;λ,μ)
            for _ in range(p['inner']):
                if count() >= n:
                    break

                # 1) eval objective + constraints
                ci = c(x)           # cost 1
                fx = f(x)           # cost 1
